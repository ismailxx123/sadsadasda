...
app.get("/", (request, response) => {
    console.log(`Ping hazır Valorant Turkey aktif.`);
...
client.on('message', msg => {
  if (msg.content.toLowerCase() === '!davet') {
    msg.reply('Valorant Turkey Sınırsız Link:https://discord.gg/EWzwRTA');
  }
});
...
client.on('guildMemberAdd', member => {
  ...
  const channel = member.guild.channels.find('name', '❤fearless-giriş-çıkış');// 'notech-log' log ismidir. değiştirebilirsiniz. belirttiğiniz isme giriş çıkış gösterecektir.
  if (!channel) return;
...
client.on('guildMemberAdd', member => {
  ...
  const channel = member.guild.channels.find('name', '📢┇ʜᴏşɢᴇʟᴅɪɴɪᴢ');// 'notech-log' log ismidir. değiştirebilirsiniz. belirttiğiniz isme giriş çıkış gösterecektir.
  if (!channel) return;
  const embed = new Discord.RichEmbed()
  .setColor('0x00cc44')
  .setAuthor(client.user.username, client.user.avatarURL)
